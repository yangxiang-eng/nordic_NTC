...
static nrf_saadc_value_t     m_buffer_pool[SAMPLES_IN_BUFFER][SAMPLES_IN_BUFFER];
...
static void saadc_done_callback(nrf_drv_saadc_evt_t const * p_event)
{
    if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
        ...
        int currentRate = 0;
        if (p_event->data.done.size < SAMPLES_IN_BUFFER)
        {
            saadc_driver_stop();
...
static void saadc_driver_start(void)
{
    ...
    /*
    err_code = nrf_drv_saadc_buffer_convert(m_buffer_pool[2], 2);
    APP_ERROR_CHECK(err_code);
    */
...
static void measure_delay_timer_handler(void*p)
{
    ...
    else if (cmdType == ADC_SUCCESS)
    {
        ...
        NRF_LOG_INFO("averageRATE :%d , gapShotAverateRate :%d \n",average1Rate,gapShotAverateRate);
